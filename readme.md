...
- 批量检测：读取文本文件中的服务器地址（支持 `IP:端口` 或 `IP 端口` 格式）
...
+---------------------------+------+------+-------------+---------+--------------------------------------+------------+
| mc.hypixel.net:25565      | 是   | 是   | Paper       | 1.8.9   | §aHypixel Network §l§e✪ §r§a§l5.4k   | 43214/50000 |
| play.cubecraft.net:25565  | 是   | 是   | Spigot      | 1.20.4  | §l§bCubeCraft Games                   | 1234/2000   |
...
├── README.md
├── requirements.txt
├── mc_bulk_checker.py      # 主入口
├── batch_runner.py         # 并发执行与重试
├── checker.py              # 单服务器检测逻辑
├── output.py               # 表格输出
└── utils.py                # 通用工具函数
